...
if [ -z "$IACT_DOMAIN" ]; then
  echo "Uso:"
  echo "  Headers/Cookie:  bash log4.sh https://alvo -i DOMINIO.oast.pro"
  echo "  GET:             bash log4.sh https://alvo -i DOMINIO.oast.pro -m GET -d 'param=PAYLOAD'"
  echo "  POST:            bash log4.sh https://alvo -i DOMINIO.oast.pro -m POST -d 'username=PAYLOAD&password=123'"
  echo "  Lista:           bash log4.sh targets.txt -i DOMINIO.oast.pro"
  exit 1
fi
...
run_scan() {
  ...
  # Remove \r se vier de arquivo Windows
